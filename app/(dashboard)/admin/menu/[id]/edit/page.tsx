...
export default function EditMenuItemPage() {
  ...
  const { data: item, isLoading } = trpc.menu.getById.useQuery({ id })
  ...
  useEffect(() => {
    if (item) {
      reset({
        ...
        ingredients: item.ingredients?.map((i: string) => ({ value: i })) || [{ value: "" }],
        allergens: item.allergens?.map((a: string) => ({ value: a })) || [],
  ...
  const onSubmit = (data: MenuItemFormData) => {
    updateItem.mutate({
      ...
      data: {
        ...
        allergens: data.allergens?.map((a: any) => a.value) || [],
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-serif text-amber-400">Редактирование блюда</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-zinc-900 rounded-xl shadow-xl border border-zinc-800 p-6 space-y-6">
        ...
        <div className="flex gap-6">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" {...register("isAvailable")} className="w-4 h-4 rounded border-zinc-600 bg-zinc-800 text-amber-500 focus:ring-amber-500" />
            <span className="text-sm text-zinc-300">В наличии</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" {...register("isPopular")} className="w-4 h-4 rounded border-zinc-600 bg-zinc-800 text-amber-500 focus:ring-amber-500" />
